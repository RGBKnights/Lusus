...
export class CubitText extends React.Component {
  ...
  singleLine(teamColor, rankColor, name) {
    return <g transform="scale(0.016,0.016) translate(30,30)">
        <title>{this.props.name}</title>
        <text style={{strokeWidth:0, fill: teamColor}} textAnchor="middle" dominantBaseline="central">{name}</text>
  ...
  render() {
    ...
    if(parts.length === 2) {
      ...
      return this.singleLine(teamColor, rankColor, this.props.name);
    }
